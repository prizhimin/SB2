...
def check_user_department(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        user_departments = InvestigationUserDepartment.objects.filter(user=request.user)
        if user_departments.count():
            return view_func(request, *args, **kwargs)
        else:
            return render(request, 'investigations/access_denied_page.html')
    return wrapper
...
def check_user_department(view_func):
    ...
    def wrapper(request, pk, *args, **kwargs):
        ...
        report = get_object_or_404(Investigation, id=pk)
